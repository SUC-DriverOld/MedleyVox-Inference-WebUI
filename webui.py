...
def change_to_audio_infer():
    return (gr.Button("Input audio files to inference", variant="primary", visible=True),
            gr.Button("Input folder to inference", variant="primary", visible=False))

def change_to_folder_infer():
    return (gr.Button("Input audio files to inference", variant="primary", visible=False),
            gr.Button("Input folder to inference", variant="primary", visible=True))
...
def inference_folder_fn(model_name, use_overlapadd, use_gpu, separate_storage, output_format, vad_method, spectral_features, ola_window_len, ola_hop_len, w2v_nth_layer_output, use_ema_model, mix_consistent_out, reorder_chunks, skip_error, folder_input, store_dir):
    if (use_overlapadd == "w2v" or use_overlapadd == "w2v_chunk") and not os.path.exists(os.path.join(PRETRAINED_MODEL_DIR, "xlsr_53_56k.pt")):
        return "Please download the pretrained model xlsr_53_56k.pt from https://dl.fbaipublicfiles.com/fairseq/wav2vec/xlsr_53_56k.pt and put it in the 'pretrained' folder!"
    if not model_name:
...
def stop_inference_fn():
    for process in multiprocessing.active_children():
        if process.name == "MedleyVox_Inference":
            ...
            return "Inference process stopped by user."
    return "No active inference process."

def webui():
    with gr.Blocks() as demo:
        gr.Markdown(value='''<div align="center"><font size=6><b>MedleyVox-Inference-WebUI</b></font></div>''')
        gr.Markdown(value='''Medley Vox is a [dataset for testing algorithms for separating multiple singers](https://arxiv.org/pdf/2211.07302) within a single music track. Also, the [authors of Medley Vox](https://github.com/jeonchangbin49/MedleyVox) proposed a neural network architecture for separating singers. However, unfortunately, they did not publish the weights. Later, their training process was [repeated by Cyru5](https://huggingface.co/Cyru5/MedleyVox/tree/main), who trained several models and published the weights in the public domain. Now this WebUI is created to use the trained models and weights for inference. Here are some precautions:<br>1. Put the [downloaded models](https://huggingface.co/Cyru5/MedleyVox) in the 'checkpoints' folder in folder format, with each model folder containing a model file (.pth) and its corresponding configuration file (.json).<br>2. If you use overlapadd and the choice of model is 'w2v' or 'w2v_chunk', you need to download the pretrained model [xlsr_53_56k.pt](https://dl.fbaipublicfiles.com/fairseq/wav2vec/xlsr_53_56k.pt) and put it in the 'pretrained' folder.<br>3. At present, the audio output sampling rate supported by the model is 24000kHz and cannot be changed. To solve this, you can use [AudioSR](https://github.com/haoheliu/versatile_audio_super_resolution), [Apollo](https://github.com/JusperLee/Apollo), or [Music Source Separation Training](https://github.com/ZFTurbo/Music-Source-Separation-Training) for audio super-resolution.<br>4. When using WebUI on cloud platforms or Colab, please place the audio to be processed in the 'inputs' folder, and the processing results will be stored in the 'results' folder. The 'Select folder' and 'Open folder' buttons are invalid in the cloud.''')
        with gr.Accordion(open=True, label="Common options"):
            with gr.Row():
                with gr.Column():
                    model_name = gr.Dropdown(label="Select Model", info="Select which model you want to use.", choices=model_list, value=None, interactive=True, scale=4)
                    use_overlapadd = gr.Radio(label="Use overlapadd", info="Use overlapadd functions, ola, ola_norm, w2v will work with ola_window_len, ola_hop_len argugments. w2v_chunk and sf_chunk is chunk-wise processing based on VAD, so you have to specify the vad_method args. If you use sf_chunk (spectral_featrues_chunk), you also need to specify spectral_features.\nIf the input is too long, it may be impossible to inference due to lack of VRAM. In that case, use 'use_overlapadd'. Among the 'use_overlapadd' options, 'ola', 'ola_norm', and 'w2v' all work well. Use w2v_chunk or sf_chunk if these fail or as desired. You can also try experimenting with 'vad_method' options spec and webrtc when using either of the '_chunk' methods. Chunking has proven to be very useful therefore it is on by default.", choices=["None", "ola", "ola_norm", "w2v", "w2v_chunk", "sf_chunk"], value="ola", interactive=True)
                with gr.Column():
                    use_gpu = gr.Checkbox(label="Use GPU", info="Use GPU for inference.", value=True, interactive=True)
                    separate_storage = gr.Checkbox(label="Save results in separate folders", info="Save results in separate folders with the same name as the input file.", value=False, interactive=True)
                    skip_error = gr.Checkbox(label="Skip error files", info="Skip error files while separating instead of stopping.", value=True, interactive=True)
                    output_format = gr.Radio(label="Output format", info="Select the output format.", choices=["wav", "flac", "mp3"], value="wav", interactive=True)
        with gr.Accordion(open=False, label="[Click to expand] Advanced options"):
            with gr.Row():
                with gr.Column():
                    vad_method = gr.Radio(label="VAD method", info="What method do you want to use for 'voice activity detection (vad) -- split chunks -- processing. Only valid when 'w2v_chunk' or 'sf_chunk' for args.use_overlapadd.", choices=["spec", "webrtc"], value="spec", interactive=True)
                    spectral_features = gr.Radio(label="Spectral features", info="What spectral feature do you want to use in correlation calc in speaker assignment (only valid when using sf_chunk)", choices=["mfcc", "spectral_centroid"], value="mfcc", interactive=True)
                    ola_window_len = gr.Number(label="OLA window length", info="OLA window size in [sec], only valid when using ola or ola_norm. Set 0 to use the default value (None).", value=0, interactive=True, step=0.01)
                    ola_hop_len = gr.Number(label="OLA hop length", info="OLA hop size in [sec], only valid when using ola or ola_norm. Set 0 to use the default value (None).", value=0, interactive=True, step=0.01)
                with gr.Column():
                    w2v_nth_layer_output = gr.Textbox(label="Wav2Vec nth layer output", info="Wav2Vec nth layer output, only valid when using w2v or w2v_chunk. For example: 0 1 2 3, default: 0", value="0", interactive=True)
                    use_ema_model = gr.Checkbox(label="Use EMA model", info="Use EMA model or online model? Only vaind when args.ema it True (model trained with EMA).", value=True, interactive=True)
                    mix_consistent_out = gr.Checkbox(label="Mix consistent output", info="Only valid when the model is trained with mixture_consistency loss.", value=True, interactive=True)
                    reorder_chunks = gr.Checkbox(label="Reorder chunks", info="OLA reorder chunks. Only valid when using ola or ola_norm.", value=True, interactive=True)
        with gr.Row():
            with gr.Column():
                with gr.Tabs():
                    with gr.TabItem(label="Input audio files") as audio_tab:
                        audio_input = gr.Files(label="Input one or more audio files", type="filepath")
                    with gr.TabItem(label="Input folder path") as folder_tab:
                        folder_input = gr.Textbox(label="Input folder path", info="Audio files in the folder will be used for inference.", value="inputs/", interactive=True, scale=4)
                        with gr.Row():
                            select_input_dir = gr.Button("Select folder")
                            open_input_dir = gr.Button("Open folder")
            with gr.Row():
                with gr.Tabs():
                    with gr.TabItem(label="Output folder path"):
                        store_dir = gr.Textbox(label="Output folder path", info="Audio files will be saved in this folder.", value="results/", interactive=True, scale=4)
                        with gr.Row():
                            select_store_btn = gr.Button("Select folder")
                            open_store_btn = gr.Button("Open folder")
        inference_audio = gr.Button("Input audio files to inference", variant="primary", visible=True)
        inference_folder = gr.Button("Input folder to inference", variant="primary", visible=False)
        output_message = gr.Textbox(label="Output message", interactive=False)
        stop_inference = gr.Button("Forced stop inference", variant="stop")
